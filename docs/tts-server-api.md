...
# 浏览器播放：拼 token
# /static/tts/word/english/English_PassionateWarrior/1f/3870be27.mp3?token=<TTS_TOKEN>
```
...
| `API_TOKEN` | `<TTS_TOKEN>` | **公网部署前必改**，长度 ≥ 32 字符 |
...
class TTSClient:
    def __init__(self, base="http://127.0.0.1:8000", token="<TTS_TOKEN>"):
        self.base = base
        self.token = token
...
// 用法
const tts = new TTSStream("http://127.0.0.1:8000", "<TTS_TOKEN>");
