...
npm run test:all         # 一键：初始化 e2e.db → 起 3100 测试服务 → E2E 14 流程 → vitest → 90 天模拟 → 摘要
...
测试文件：`tests/e2e/01~14-*.spec.ts`（数字前缀强制顺序，串行 workers=1；11 突击模式 / 12 FSRS 细节 / 13 健壮性 / 14 规模）、`tests/fsrs/fsrs-unit.test.ts`（18 用例：含突击保留率/自定义参数/maxInterval 封顶/边界值）、`scripts/test/fsrs-simulate.mjs`（90 天模拟，6 项硬指标）、`playwright.config.ts`、`vitest.config.ts`。
