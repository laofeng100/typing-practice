...
// 获取已有标题避免重复
const existing = new Set((await db.listeningArticle.findMany({select:{title:true}})).map(a => a.title.toLowerCase()))
console.log('已有文章:', existing.size, '篇')

const juniorTodo = [
  {c:'日常对话',t:'At the Library'},{c:'日常对话',t:'Talking About Hobbies'},{c:'日常对话',t:'Buying Clothes'},
  {c:'故事讲述',t:'The Lost Cat'},{c:'故事讲述',t:'A Forest Adventure'},{c:'故事讲述',t:'The Power of Friendship'},
  {c:'故事讲述',t:'A Rainy Day Surprise'},{c:'故事讲述',t:'The Boy Who Cried Wolf'},
  {c:'新闻播报',t:'School Sports Meeting'},{c:'新闻播报',t:'Environmental Protection'},{c:'新闻播报',t:'Science Competition'},{c:'新闻播报',t:'Community Volunteer Service'},
  {c:'科普知识',t:'How Volcanoes Form'},{c:'科普知识',t:'The Water Cycle'},{c:'科普知识',t:'Photosynthesis'},
  {c:'文化介绍',t:'Chinese Spring Festival'},{c:'文化介绍',t:'Western Table Manners'},{c:'文化介绍',t:'Greetings Around the World'},
  {c:'诗歌朗诵',t:'Friendship'},{c:'诗歌朗诵',t:'Dreams'},
]
const seniorTodo = [
...
async function gen(stage: string, items: {c:string,t:string}[], diff: string, wc: string, qc: number) {
  for (const a of items) {
    // 检查是否已有同名（跨学段不算重复）
    const stageExists = await db.listeningArticle.findFirst({ where: { stage, title: { contains: a.t } } })
    if (stageExists) { console.log(stage + ' ' + a.t + ' 已存在，跳过'); continue }
    console.log(stage + ' ' + a.t)
    try {
      const r = await zai.chat.completions.create({messages:[{role:'user',content:'Generate '+stage+' English listening article. Topic:'+a.t+'. '+wc+' words. '+qc+' questions '+(qc===3?3:4)+' options. JSON only:{"title":"","content":"","contentZh":"","wordCount":0,"questions":[{"q":"","options":["A. ","B. ","C. "],"answer":0,"explain":""}],"vocabulary":[]}'}],thinking:{type:'disabled'}})
...
await gen('初中', juniorTodo, 'A2', '100-180', 4)
await gen('高中', seniorTodo, 'B1', '150-250', 5)
